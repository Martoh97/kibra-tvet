...
const Courses = () => {
  ...
  const filteredCourses = coursesData.filter((course) => {
    const matchesCategory = selectedCategory === "All Courses" || course.level === selectedCategory;
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <section className="relative bg-gradient-to-r from-primary to-primary/80 text-primary-foreground py-20">
        <div className="container mx-auto px-4">
          ...
          <p className="text-lg md:text-xl opacity-90">Discover our wide range of courses designed to equip you with practical skills</p>
        </div>
      </section>
      ...
      <main className="flex-1 py-12">
        <div className="container mx-auto px-4">
          ...
          {/* Category Filters */}
          <div className="mb-8">
            <h2 className="text-sm font-semibold text-muted-foreground mb-3">Filter by Level</h2>
          ...
          {/* Department Filters */}
          <div className="mb-8">
            <h2 className="text-sm font-semibold text-muted-foreground mb-3">Filter by Department</h2>
            <div className="flex flex-wrap gap-2">
              ...
              <Card key={course.id} className="hover:shadow-lg transition-shadow">
                <CardHeader>
                  <div className="flex items-start justify-between gap-2 mb-2">
                    ...
                    <Badge className="bg-accent text-accent-foreground">{course.accreditation}</Badge>
                  </div>
                  <CardTitle className="text-lg leading-tight">{course.name}</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="space-y-2 text-sm text-muted-foreground">
                    <p><span className="font-medium text-foreground">Department:</span> {course.department}</p>
